...
class ExperimentManager:
    ...
    def _preprocess_hyperparams(
        ...
        if "pref_learning" in hyperparams.keys():
            if hyperparams["pref_learning"]["active"]:
                ...
                hc = get_preference_human_critic(hyperparams, self.env_name)
                pref_callbacks = get_preference_callbacks(hyperparams, hc, self.env_name, self.args.prefcallback)
                pref_wrappers = get_preference_wrappers(hyperparams, hc, self.env_name, self.n_envs)
        ...
        env_wrapper = get_wrapper_class(hyperparams, pref=pref_wrappers)
    ...
    def create_envs(self, n_envs: int, eval_env: bool = False, no_log: bool = False) -> VecEnv:
        ...
        if self.env_name == "Social-Nav-v1":
            ...
            worker_id = random.randint(0, 20000)
            if self.args.workerid != -1:
                worker_id += self.args.workerid
            unity_env = UnityEnvironment('envs/politefinal/socialnav1', side_channels=[channel], worker_id=worker_id,
                                         no_graphics=True)
            # unity_env = UnityEnvironment('envs/politefinallines/socialnav1lines', side_channels=[channel], worker_id=worker_id, no_graphics=False)
            # unity_env = None
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        if "hc" in kwargs:
            print("Got in here: " + self.algo + "_pref")
            sampled_hyperparams = HYPERPARAMS_SAMPLER[self.algo + "_pref"](trial)
        else:
        ...
        if self.algo == "ppo" and "hc" in kwargs:
            print("Getting callbacks for pref")
